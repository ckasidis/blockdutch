...
describe("DutchAuction", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should sell token at price no lower than reserved price", async function () {
      ...
      expect(Number(tokenPrice3)).to.greaterThanOrEqual(Number(tokenReservedPrice));
    ...
    it("Should distribute token no more than the supply", async function () {
      ...
      expect(Number(bidder1Tokens)/10e17).to.be.greaterThan(666).and.lessThan(668);
      expect(Number(bidder2Tokens)/10e17).to.be.greaterThan(331).and.lessThan(333);
  ...
  describe("Withdrawals", function () {
    ...
    describe("Case: Demand > Supply", function (){
      it("Should sold tokens at correct clearing price", async function () {
        ...
        expect((Number(finalClearingPrice)/10e17).toFixed(2)).to.equal("1.25");
      ...
      it("Should refund bids if demand exceed token supply", async function () {
        ...
        expect((Number(prevBalanceBidder1)/10e17 - 100).toFixed(0)).to.equal((Number(balanceBidder1)/10e17).toFixed(0));
        // owner received ethers
        expect(prevBalanceOwner).to.lt(balanceOwner);
